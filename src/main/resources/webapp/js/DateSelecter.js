...
function calendar() //主调函数
{
    ...
    if  (!WebCalendar.timeShow) 
    	WebCalendar.format== "yyyymmdd"?WebCalendar.dateReg = /^(\d{4})(-|\/|.|)(\d{2})\2(\d{2})$/:WebCalendar.dateReg = /^(\d{1,4})(-|\/|.)(\d{1,2})\2(\d{1,2})$/;
    else 
    	WebCalendar.format== "yyyymmdd"?WebCalendar.dateReg = /^(\d{4})(-|\/|.|)(\d{2})\2(\d{2}) (\d{1,2}):(\d{1,2}):(\d{1,2})$/:WebCalendar.dateReg = /^(\d{1,4})(-|\/|.)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})$/;
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        ...
        if(WebCalendar.MinValue!="" && WebCalendar.MinValue>returnValue )return;
        if(WebCalendar.MaxValue!="" && WebCalendar.MaxValue<returnValue )return;
