...
function MzTreeView(Tname)
{
  ...
  this.toString = function()
  {
    ...
    if(rootCN.length>0)
    {
      ...
      setTimeout(this.name +".atRootIsEmpty();"+ this.name +".focusClientNode('"+ rootCN[0].id +"'); ",100);
    }
...
//    }
    var htmlStr= "<DIV class='MzTreeView' id='MzTreeView_"+ this.name +"'"+
	    "onclick='"+ this.name +".clickHandle(event)' "+
	    "ondblclick='"+ this.name +".dblClickHandle(event)' ";
    if (this.useArrow)  //使用方向键控制跳转到上级下级父级子级节点
    {
        htmlStr+=" onkeydown='"+ this.name +".onkeydown(event)'";
    }
      htmlStr+=">"+ str +"</DIV>";
      return htmlStr;
  };
...
MzTreeView.prototype.dataFormat = function()
{
  ...
  for (var id in this.nodes) 
    ...
    {
        a[a.length] = id;
//         document.getElementById("cid").value+=id+"="+ this.nodes[id]+"\r\n";
    }   
...
MzTreeView.prototype.load = function(id)
{
  ...
  if(node)
  {
    var sid = node.sourceIndex;
    if (sid.indexOf(d)>=0) {
        sid = sid.substr(sid.indexOf(d) + d.length);
    }
  var reg = new RegExp("(^|"+_d+")"+ sid +d+"[^"+_d +"]+?("+_d+"|$)", "g");
...
{
  if(!this.nodes[sourceIndex])return;
  this.index++;
  ...
  var value= this.getAttribute(source, "value");
  ...
  this.node[this.index] =
  {
    ...
    "value ": value ?value :""
     ...
     this.node[this.index].value=value ;
...
{
    if(!this.nodes[node.sourceIndex])return;
  var source = this.nodes[node.sourceIndex];
  var target = this.getAttribute(source, "target");
  var data = this.getAttribute(source, "data");
  var url  = this.getAttribute(source, "url");
  var disabled=this.getAttribute(source, "disabled");
   var value=this.getAttribute(source, "value");
   
    if(!value) value = this.value;
  if(!url) url = this.url;
  if(!disabled) disabled = this.disabled;
...
//    if(node.sourceIndex=="root_root2")
//    {
//        alert(HCN+"__"+ node.icon+"__"+node.iconExpand);
//        
//    }
  var HTML = "<DIV noWrap='True'";
  if(disabled && disabled=='true'){HTML+=" class=\"disabled\"><NOBR>";}else{HTML+=" ><NOBR>";}
  if(!isRoot)
  {
  ...
  HTML += "<IMG "+
    ...
    "target='"+ target +"' "+
    "title='"+ node.hint +"' ";
    if(value)HTML +=  "value='"+ node.value +"' ";
   ...
   {
   	  HTML +="class='MzTreeview' hideFocus "+
    		"href='"+ url +"' onfocus=\""+ this.name +".focusLink('"+ id +"')\"  onclick=\"return "+ this.name +".nodeClick('"+ id +"')\"";
   }
...
MzTreeView.prototype.buildNode = function(id)
{
  if(this.node[id] && this.node[id].hasChild)
  {
    ...
    if(tcn[i] && this.nodes[tcn[i].sourceIndex])
      str += this.nodeToHTML(tcn[i], i==tcn.length-1);
    var temp = this.getElementById(this.name +"_tree_"+ id);
    if(temp)temp=temp.childNodes;
   if(temp)temp[temp.length-1].innerHTML = str;
...
if(!this.node[id])return;
  ...
  var js=this.getAttribute(source, "method");


if(js.indexOf(")")>0)
{
	if((js.indexOf(")")-js.indexOf("("))>1) 
	{
	   js=js.substring(0,js.indexOf(")"))+",this,this.node["+id+"])";
	}
	else
	{
		js=js.substring(0,js.indexOf(")"))+"this,this.node["+id+"])";
...
//展开树的所有节点
MzTreeView.prototype.expandAll = function(alt)
{
  if(this.totalNode>500 && alt) if(
...
MzTreeView.prototype.addnode=function(pid,sid,source) 
{ 
	  var d = this.divider, _d = this._d;
    var sourceIndex=pid+d+sid;
    var regS=""+ d+pid+"$" ;
    ...
    {	
        if(this.node[i])
        {
    	if(new RegExp(regS, "g").test(this.node[i].sourceIndex)) 
    	{ 
		     parentId =i ; 	
	    	 break;
    	} 	
    ...
    if(parentId)
    {
        this.Bparent(pid,parentId,sid,source);
    }
    else
    {
//    alert(sourceIndex);
    ...
    //setTimeout(this.name +".focus('"+ sid +"',true); "+this.name +".buildNode('this.currentNode.id',true); ",100); 
//    this.focus(sid,true);
//    node=this.currentNode;
//    alert(node.id);
//    this.buildNode(node.id);
//    this.expand(node.id);
 //    this.Bparent(pid,sourceIndex);
        //setTimeout(this.name +".Bparent('"+ pid +"','"+sourceIndex+"'); ",100); 
}
 
MzTreeView.prototype.Bparent=function(pid,parentId,sid,source)
{
    var d = this.divider, _d = this._d;
    var sourceIndex=pid+d+sid;

    if(!this.node[parentId])return;
    if(this.node[parentId].hasChild)
    {
        node=this.nodeInit(sourceIndex, parentId); 
        this.node[parentId].childNodes[this.node[parentId].childNodes.length]=node;
        this.Bchild(parentId);
        this.focus(sid,true);
        this.expand(this.currentNode.id, false);
    }
    else
    {  	
        ...
        if(!this.node[parentId].childNodes)this.node[parentId].childNodes =[];
//        node=this.nodeInit(sourceIndex, parentId); 
 //       this.node[parentId].childNodes[this.node[parentId].childNodes.length]=node;

        var path = this.getPath(pid);
        if(path[0]!=this.rootId)
        {
            alert("节点 "+ sourceId +" 没有正确的挂靠有效树节点上！\r\n"+
            "节点 id 序列 = "+ path.join(this.divider));
...
MzTreeView.prototype.Bchild=function(id,all)
{ 
if(this.node[id] && this.node[id].hasChild)
{
    this.buildNode(id);
    for(node in this.node[id].childNodes)
    {
        var node=this.node[id].childNodes[node];
        if(node && (all || node.isExpand))  // && 
        {
            this.buildNode(node.id);
            this.expand(node.id);
        }
        if(node.hasChild && node.childNodes.length>0)this.Bchild(node.id,all);
     //   alert(node.id+"___"+node.text);
    }
  }
}
MzTreeView.prototype.delnode=function(id)
{
    var nodeIndexId;   
	var d = this.divider, _d = this._d;
	var sourceIndex;
    ...
    {
        nodeIndexId=this.node.currentNode.id;
    }
    
    if(nodeIndexId) 
    {
         sourceIndex=this.node[nodeIndexId].sourceIndex;
       // alert("nodeIndexId:"+nodeIndexId+";sourceIndex:"+sourceIndex);
        var ishasChild=this.node[nodeIndexId].hasChild;
       // alert(ishasChild);
        if(ishasChild)
        {
            this.expand(nodeIndexId);
            this.Bchild(nodeIndexId,true);
            this.focus(id,true);
//            this.expand(nodeIndexId, false);
            this.removenodes(nodeIndexId); 
           //var cl=this.node[nodeIndexId].childNodes.length;         
           /* for(ins=0;ins<cl;ins++)
            {           
                var node= this.node[nodeIndexId].childNodes[ins];
                if(node && this.nodes[node.sourceIndex])
                {
                   //this.nodes[node.sourceIndex]=null;
                   //node=null; 
                   //  alert(node.sourceIndex);   
                   this.delnode(node.sourceIndex.substring(node.sourceIndex.indexOf(d)+d.length));   
                   this.node[nodeIndexId].hasChild=this.node[nodeIndexId].childNodes.length-ins-1>0?true:false;                   
                   this.node[nodeIndexId].childNodes.shift() ;
                }
                alert(cl+"__"+ins);
            }
            */            
        }
        this.nodes[sourceIndex]=undefined;
        
        var parentId=sourceIndex.substring(0,sourceIndex.indexOf(d))
        var parentIndexId;
        var regS=""+ d+parentId+"$" ; 
        for(i in this.node)
        {	
        ...
        for(i=0;i<this.node[parentIndexId].childNodes.length;i++)
        {
            var node=this.node[parentIndexId].childNodes[i];
            if(this.nodes[node.sourceIndex]==undefined)
            {
                this.node[node.id]=undefined;
                this.node[parentIndexId].childNodes[i]=undefined;//.shift() ;;
                this.node[parentIndexId].hasChild=this.node[parentIndexId].childNodes.length-1>0?true:false;
            }
        }
        var cnodes=this.node[parentIndexId].childNodes;
        this.node[parentIndexId].childNodes=[];
        for(i=0;i<cnodes.length;i++)
        {
            var node=cnodes[i];
            if(node)
            {
                this.node[parentIndexId].childNodes[this.node[parentIndexId].childNodes.length]=node;
            }
        }
        this.node[parentIndexId].hasChild=this.node[parentIndexId].childNodes.length>0?true:false;
        sourceIndex=this.node[parentIndexId].sourceIndex;
        
    //    document.getElementById("cid").value+="\r\n"+this.names;
        this.dataFormat();

        // alert("parentIndexId:"+parentIndexId+";sourceIndex:"+sourceIndex);
        if(this.node[parentIndexId].hasChild)
        {
            this.Bchild(parentIndexId);
            this.focus(parentId,true);
            this.expand(this.currentNode.id, false);
        }
        else
        {   
            parentId=sourceIndex.substring(0,sourceIndex.indexOf(d))
            regS=""+ d+parentId+"$" ; 
            for(i in this.node)
            {	
                if(this.node[i])
           ...
           // alert("parentIndexId2:"+parentIndexId+";sourceIndex2:"+sourceIndex);
            this.Bchild(parentIndexId);
            this.focus(parentId,true);
            this.expand(this.currentNode.id, false);
        }
//        document.getElementById("cid").value+="\r\n"+this.names;
 ...
 MzTreeView.prototype.removenodes=function(id)
 {
//       var d = this.divider, _d = this._d;
//       this.Bchild(id);
//       this.focus(this.node[id].sourceIndex.substring(this.node[id].sourceIndex.indexOf(d)+d.length),true);
       if(this.node[id] && this.node[id].hasChild)
       {
           while(this.node[id].childNodes.length)
           {
             var node=this.node[id].childNodes[0];
             this.removenodes(node.id); 
             this.node[id].childNodes.shift() ;
             this.node[id].hasChild=this.node[id].childNodes.length-1>0?true:false; 
           }  
       }
       this.nodes[this.node[id].sourceIndex]=undefined;
//       document.getElementById("cid").value+=this.node[id].sourceIndex+"="+ this.nodes[this.node[id].sourceIndex]+"\r\n";
       this.node[id]=undefined; 
 }

MzTreeView.prototype.update=function(cid,source)
{
    var _d = this._d, d = this.divider;
    var pid,cIndexid,pIndexId;
    var str = "(^|"+_d+")([^"+_d+d+"]+"+d+ cid +")("+_d+"|$)";
    if (new RegExp(str).test(this.names))
    {
        pid = RegExp.$2.substring(0, RegExp.$2.indexOf(d));
    }
    if(pid)
    {
        var regS=""+ d+cid+"$" ; 
        var regP=""+ d+pid+"$" ; 
        this.focus(cid,true);
        this.expand(this.currentNode.id);
        for(i in this.node)
        {	
            if(this.node[i])
            {
            	...
            	else if(new RegExp(regP, "g").test(this.node[i].sourceIndex)) 
            	{
	                pIndexId =i ; 	
	                if(cIndexid)break;
            	}
        ...
        var sourceIndex=this.node[cIndexid].sourceIndex;
        this.nodes[sourceIndex]="index:"+ cIndexid +";"+source;
        for(i=0;i<this.node[pIndexId].childNodes.length;i++)
        {
            if(this.node[pIndexId].childNodes[i].id==cIndexid)
            {
                var source= this.nodes[sourceIndex], d = this.divider;
                var text  = this.getAttribute(source, "text");
                var url  = this.getAttribute(source, "url");
                ...
                this.node[pIndexId].childNodes[i].text=text;
                this.node[pIndexId].childNodes[i].href=url;
                this.node[pIndexId].childNodes[i].data=data;
                this.node[pIndexId].childNodes[i].hint=hint;
                this.node[pIndexId].childNodes[i].value=value;
                break;
            }
        }        
        this.Bchild(pIndexId);
    }
    else
    {
        alert("不存在ID为"+cid+"的节点");
    }   
}
