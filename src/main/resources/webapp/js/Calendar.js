...
{  
this.style = {  
  borderColor    : "#5A84D6", //边框颜色  
  headerBackColor   : "#5A84D6", //表头背景颜色  
  headerFontColor   : "#ffffff", //表头字体颜色  
  bodyBarBackColor  : "#D0E8FF", //日历标题背景色  
...
var _arr = date.split('-'); 
var zeromonth = _arr[1]-1;
var zeroday = _arr[2];
zeromonth = changeZero(zeromonth);
zeroday = changeZero(zeroday);
  var _date = new Date(_arr[0],zeromonth,zeroday);  
...
{  
  ...
  var zeromonth = changeZero(this.getMonth()+1);
  var zeroday = changeZero(this.getDate());
	
  if(fs.length == 1)  
  {  
   fs='';
   return this.getFullYear() + fs + zeromonth + fs + zeroday;  
  }
	
 

  fs = fs.replace("yyyy",this.getFullYear());  
  fs = fs.replace("mm",zeromonth);  
  fs = fs.replace("dd",zeroday);  
  return fs;  
} 

function changeZero(ss)
{
	if(parseInt(ss)<10)
	{
		ss = "0" + ss;
