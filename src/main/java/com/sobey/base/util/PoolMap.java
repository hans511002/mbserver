...
/**
 * 
 * The <code>PoolMap</code> maps a key to a collection of values, the elements of which are managed by a pool. In
 * effect, that collection acts as a shared pool of resources, access to which is closely controlled as per the
 * semantics of the pool.
 * 
 * <p>
 * In case the size of the pool is set to a non-zero positive number, that is used to cap the number of resources that a
 * pool may contain for any given key. A size of {@link Integer#MAX_VALUE} is interpreted as an unbounded pool.
 * </p>
 * 
 * @param <K>
...
public class PoolMap<K, V> implements Map<K, V> {
	...
	/**
	 * The <code>ReusablePool</code> represents a {@link PoolMap.Pool} that builds on the {@link LinkedList} class. It
	 * essentially allows resources to be checked out, at which point it is removed from this pool. When the resource is
	 * no longer required, it should be returned to the pool in order to be reused.
	 * 
	 * <p>
	 * If {@link #maxSize} is set to {@link Integer#MAX_VALUE}, then the size of the pool is unbounded. Otherwise, it
	 * caps the number of consumers that can check out a resource from this pool to the (non-zero positive) value
	...
	/**
	 * The <code>RoundRobinPool</code> represents a {@link PoolMap.Pool}, which stores its resources in an
	 * {@link ArrayList}. It load-balances access to its resources by returning a different resource every time a given
	 * key is looked up.
	 * 
	 * <p>
	 * If {@link #maxSize} is set to {@link Integer#MAX_VALUE}, then the size of the pool is unbounded. Otherwise, it
	 * caps the number of resources in this pool to the (non-zero positive) value specified in {@link #maxSize}.
	 * </p>
	 * 
	 * @param <R>
	 *            the type of the resource
	...
	/**
	 * The <code>ThreadLocalPool</code> represents a {@link PoolMap.Pool} that builds on the {@link ThreadLocal} class.
